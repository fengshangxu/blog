  ...
  <tr>
   <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
	    ...
	    <td align="center"><?php echo ($v["add_time"]); ?></td>
	      ...
	      <a class="ajax" ajaxid="<?php echo ($v['id']); ?>" title="还原" style="cursor: pointer" ><img src="/blog/Public/admin/images/yes.gif" width="16" height="16" border="0" /></a>
	      <a class="ajaxdel" ajaxid="<?php echo ($v['id']); ?>" style="cursor: pointer" title="彻底删除"><img src="/blog/Public/admin/images/no.gif" width="16" height="16" border="0" /></a>
          </td>
...
<script>
	$('.ajax').click(function(){
		...
		$.ajax({
			type:"get",
			url:'<?php echo U("Admin/Article/recovery");?>?id='+id,
	...
	$('.ajaxdel').click(function(){
		...
		$.ajax({
			type:"get",
			url:'<?php echo U("Admin/Article/articledel");?>?id='+id,
			success:function(data){
				$this.parent().parent().remove();
			}
		});		 
